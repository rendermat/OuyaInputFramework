...
public static class OuyaInput
{
  ...
  private enum AndroidBluetoothDevice
  {
    Ouya,
    MogaPro
  }
  ...
  private class PlayerController
  {
    ...
    public float rangeMapTriggerAxis(float axisValue, OuyaAxis triggerAxis) {
      ...
      switch (triggerAxis) {
        case OuyaAxis.LT:
          if (!initAxisLT && axisValue != 0f)
            initAxisLT = true;
          break;
        case OuyaAxis.RT:
          if (!initAxisRT && axisValue != 0f)
            initAxisRT = true;
          break;
      }
  ...
  private static void mapController(PlayerController playerController) {
    ...
    switch (playerController.mapType) {
      ...
      case OuyaMapType.Unknown:
        ...
        playerController.map_LX = string.Format("Joy{0} Axis 1", player);
        playerController.invert_LX = false;
        playerController.map_LY = string.Format("Joy{0} Axis 2", player);
        playerController.invert_LY = true;
        playerController.map_RX = string.Format("Joy{0} Axis 3", player);
        playerController.invert_RX = false;
        playerController.map_RY = string.Format("Joy{0} Axis 4", player);
        playerController.invert_RY = true;
        playerController.map_DX = string.Format("Joy{0} Axis 5", player);
        playerController.invert_DX = false;
        playerController.map_DY = string.Format("Joy{0} Axis 6", player);
        playerController.invert_DY = true;
        playerController.map_LT = string.Format("Joy{0} Axis 7", player);
        playerController.invert_LT = false;
        playerController.map_RT = string.Format("Joy{0} Axis 8", player);
        playerController.invert_RT = false;
        break;
    }
#if !UNITY_EDITOR && UNITY_ANDROID
    if(playerController.mapType == OuyaMapType.GameStick_CONSOLE){
        // the GameStick controller has no triggers at all
            playerController.map_LT = null; playerController.invert_LT = false;
            playerController.map_RT = null; playerController.invert_RT = false;
    }
  ...
  private static bool SwitchSinglePlayerMap() {
    ...
    for (int i = 1; i <= playersMax; i++) {
      ...
      if (GetAxis(OuyaAxis.LX, OuyaPlayer.P01) != 0)
        return true;
      if (GetAxis(OuyaAxis.LY, OuyaPlayer.P01) != 0)
        return true;
      if (GetAxis(OuyaAxis.RX, OuyaPlayer.P01) != 0)
        return true;
      if (GetAxis(OuyaAxis.RY, OuyaPlayer.P01) != 0)
        return true;
      if (GetAxis(OuyaAxis.DX, OuyaPlayer.P01) != 0)
        return true;
      if (GetAxis(OuyaAxis.DY, OuyaPlayer.P01) != 0)
        return true;
      if (GetAxis(OuyaAxis.LT, OuyaPlayer.P01) != 0)
        return true;
      if (GetAxis(OuyaAxis.RT, OuyaPlayer.P01) != 0)
        return true;

      if (GetButton(OuyaButton.O, OuyaPlayer.P01))
        return true;
      if (GetButton(OuyaButton.U, OuyaPlayer.P01))
        return true;
      if (GetButton(OuyaButton.Y, OuyaPlayer.P01))
        return true;
      if (GetButton(OuyaButton.A, OuyaPlayer.P01))
        return true;

      if (GetButton(OuyaButton.LB, OuyaPlayer.P01))
        return true;
      if (GetButton(OuyaButton.RB, OuyaPlayer.P01))
        return true;
      if (GetButton(OuyaButton.L3, OuyaPlayer.P01))
        return true;
      if (GetButton(OuyaButton.R3, OuyaPlayer.P01))
        return true;

      if (GetButton(OuyaButton.START, OuyaPlayer.P01))
        return true;
      if (GetButton(OuyaButton.SELECT, OuyaPlayer.P01))
        return true;
      if (GetButton(OuyaButton.SYSTEM, OuyaPlayer.P01))
        return true;
    }
  ...
  public static string GetControllerName(OuyaPlayer player) {
    ...
    if (controllerNames == null || index >= controllerCount)
  ...
  public static float GetAxis(OuyaAxis axis, OuyaPlayer player) {
    ...
    if (playerIndex < 0 || playerIndex >= controllerCount)
      return 0f;
    ...
    if (playerController == null)
      ...
      switch (axis) {
        case OuyaAxis.LX:
          axisName = playerController.map_LX;
          invert = playerController.invert_LX;
          break;
        case OuyaAxis.LY:
          axisName = playerController.map_LY;
          invert = playerController.invert_LY;
          break;
        case OuyaAxis.RX:
          axisName = playerController.map_RX;
          invert = playerController.invert_RX;
          break;
        case OuyaAxis.RY:
          axisName = playerController.map_RY;
          invert = playerController.invert_RY;
          break;
      ...
      /* LT-TRIGGER */
        case OuyaAxis.LT:
          axisName = playerController.map_LT;
          invert = playerController.invert_LT;
          ...
          if (axisName == null) {
            switch (mapType) {
              ...
              case OuyaMapType.PS3_ANDROID:
              case OuyaMapType.PS3_OSX:
                if (GetButton(OuyaButton.LT, player))
                  return 1f;
                else
                  return 0f;
            }
      ...
      /* RT-TRIGGER */
        case OuyaAxis.RT:
          axisName = playerController.map_RT;
          invert = playerController.invert_RT;
				// if the trigger is treated like a button we convert button press bool flags into axis values
          if (axisName == null) {
            switch (mapType) {
              case OuyaMapType.MotionInJoy_WIN:
              case OuyaMapType.PS3_ANDROID:
              case OuyaMapType.PS3_OSX:
                if (GetButton(OuyaButton.RT, player))
                  return 1f;
                else
                  return 0f;
            }
      ...
      /* DX-AXIS */
        case OuyaAxis.DX:
          axisName = playerController.map_DX;
          invert = playerController.invert_DX;
      ...
      /* DY-AXIS */
        case OuyaAxis.DY:
          axisName = playerController.map_DY;
          invert = playerController.invert_DY;
				// if the dpad is treated like a button we convert button press bool flags into axis values
          if (axisName == null) {
            switch (mapType) {		
						#if !UNITY_EDITOR && UNITY_ANDROID
    ...
    if (axis == OuyaAxis.LT || axis == OuyaAxis.RT) {
      ...
      if (invert)
        return playerController.rangeMapTriggerAxis(-Input.GetAxisRaw(axisName), axis);
      else
        return playerController.rangeMapTriggerAxis(Input.GetAxisRaw(axisName), axis);
    }
    ...
    if (playerController != null) {
      ...
      /* ANDROID MAPPINGS */
        ...
        case OuyaMapType.XBOX360_ANDROID_wireless:
				...
				switch (button)
				{
					...
					// d-pad buttons
          case OuyaButton.DU: return GetAxis (OuyaAxis.DY, player) > 0;; // checked
          case OuyaButton.DD: return GetAxis (OuyaAxis.DY, player) < 0; // checked
          case OuyaButton.DL: return GetAxis (OuyaAxis.DX, player) < 0; // checked
          case OuyaButton.DR: return GetAxis (OuyaAxis.DX, player) > 0; // checked 
          case OuyaButton.LT: return GetAxis (OuyaAxis.LT, mapPlayer) > 0.5f;
          case OuyaButton.RT: return GetAxis (OuyaAxis.RT, mapPlayer) > 0.5f;
      ...
      /* UNKNOWN MAPPINGS */
        case OuyaMapType.Unknown:
          ...
          // ouya buttons
            case OuyaButton.O:
              return GetButton(0, buttonAction, mapPlayer);
            case OuyaButton.U:
              return GetButton(2, buttonAction, mapPlayer);
            case OuyaButton.Y:
              return GetButton(3, buttonAction, mapPlayer);
            case OuyaButton.A:
              return GetButton(1, buttonAction, mapPlayer);

          // shoulder buttons
            case OuyaButton.LB:
              return GetButton(4, buttonAction, mapPlayer);
            case OuyaButton.RB:
              return GetButton(5, buttonAction, mapPlayer);

          // stick buttons
            case OuyaButton.L3:
              return GetButton(8, buttonAction, mapPlayer);
            case OuyaButton.R3:
              return GetButton(9, buttonAction, mapPlayer);

          // d-pad buttons and trigger buttons
          // tese buttons are axis and do not give out UP or DOWN events natively
          // we use button state management and continious scanning to provide these	
            case OuyaButton.DU:
              return GetCachedButtonEvent(button, buttonAction, playerIndex);
            case OuyaButton.DD:
              return GetCachedButtonEvent(button, buttonAction, playerIndex);
            case OuyaButton.DL:
              return GetCachedButtonEvent(button, buttonAction, playerIndex);
  ...
  private static bool GetCachedButtonEvent(OuyaButton button, ButtonAction buttonAction, int playerIndex) {
    ...
    // we use button state management and continious scanning to provide these
      case OuyaButton.DU:
        switch (buttonAction) {
          case ButtonAction.DownFrame:
            return playerControllers[playerIndex].downEventDU;
          case ButtonAction.UpFrame:
            return playerControllers[playerIndex].upEventDU;
          default:
            return GetAxis(OuyaAxis.DY, player) > 0f;
        }
      case OuyaButton.DD:
        switch (buttonAction) {
          case ButtonAction.DownFrame:
            return playerControllers[playerIndex].downEventDD;
          case ButtonAction.UpFrame:
            return playerControllers[playerIndex].upEventDD;
          default:
            return GetAxis(OuyaAxis.DY, player) < 0f;
        }
      case OuyaButton.DL:
        switch (buttonAction) {
          case ButtonAction.DownFrame:
            return playerControllers[playerIndex].downEventDL;
          case ButtonAction.UpFrame:
            return playerControllers[playerIndex].upEventDL;
          default:
            return GetAxis(OuyaAxis.DX, player) < 0f;
        }
      case OuyaButton.DR:
        switch (buttonAction) {
          case ButtonAction.DownFrame:
            return playerControllers[playerIndex].downEventDR;
          case ButtonAction.UpFrame:
            return playerControllers[playerIndex].upEventDR;
          default:
            return GetAxis(OuyaAxis.DX, player) > 0f;		
        }
    // trigger buttons
    ...
    // we use button state management and continious scanning to provide these
      case OuyaButton.LT:															
        switch (buttonAction) {
          case ButtonAction.DownFrame:
            return playerControllers[playerIndex].downEventLT;
          case ButtonAction.UpFrame:
            return playerControllers[playerIndex].upEventLT;
          default:
            return GetAxis(OuyaAxis.LT, player) > triggerThreshold;	
        }
      case OuyaButton.RT:															
        switch (buttonAction) {
          case ButtonAction.DownFrame:
            return playerControllers[playerIndex].downEventRT;
          case ButtonAction.UpFrame:
            return playerControllers[playerIndex].upEventRT;
          default:
            return GetAxis(OuyaAxis.RT, player) > triggerThreshold;	
        }
    // not defined so far
      default:
        return false;
  ...
  public static float GetTrigger(OuyaTrigger trigger, OuyaPlayer player) {
    ...
    switch (trigger) {
      case OuyaTrigger.Left:
        triggerInput = GetAxis(OuyaAxis.LT, player);
        break;
      case OuyaTrigger.Right:
        triggerInput = GetAxis(OuyaAxis.RT, player);
        break;
      default:
        triggerInput = 0f;
        break;
    ...
    if (triggerInput < triggerThreshold)
      return 0f;
    else {
      switch (deadzoneType) {
        case DeadzoneType.AxialClip:
  ...
  public static float CalculateJoystickAngle(Vector2 joystickInput) {
    ...
    if (joystickInput.x < 0 && joystickInput.y > 0)
      ...
      quad = Quadrant.OnYPos;
    else if (joystickInput.x > 0 && joystickInput.y == 0)
      quad = Quadrant.OnXPos;
    else if (joystickInput.x == 0 && joystickInput.y < 0)
      quad = Quadrant.OnYNeg;
    else if (joystickInput.x < 0 && joystickInput.y == 0)
      quad = Quadrant.OnXNeg;
    else if (joystickInput.x == 0 && joystickInput.y == 0)
      quad = Quadrant.Zero;
    ...
    switch (quad) {
      case Quadrant.I:
        angle = Mathf.Atan2(joystickInput.y, joystickInput.x) * (180f / Mathf.PI);
        break;
      case Quadrant.II:
        angle = Mathf.Atan2(joystickInput.y, joystickInput.x) * (180f / Mathf.PI);
        break;
      case Quadrant.III:
        angle = (Mathf.Atan2(joystickInput.y, joystickInput.x) * (180f / Mathf.PI)) + 360;
        break;
      case Quadrant.IV:
        angle = (Mathf.Atan2(joystickInput.y, joystickInput.x) * (180f / Mathf.PI)) + 360;
        break;
